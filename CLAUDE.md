...
- Works に「あがれ！ジャンプビート」（2026）を追加し、**Works全体の一番上（最新）**に配置（`jump-beat`、2026グループ先頭＝ぺったんメイカーの前。カルーセル `order` も先頭）。タグ＝Unreal Engine / C++ / 課題制作：個人、env＝`Unreal Engine / C++`、制作期間＝1週間弱。desc＝ゲームジャム「ぷちコン」応募用に1週間弱で制作したタイミングアクションゲーム／お題「だん」から「段差」を軸にし段差をジャンプで登るのが中心／音楽に合わせて跳ぶタイミングゲームとして構成／ジャンプだけでは単調なため銃を撃てる要素を追加＝裏拍を絡めたリズムと操作難易度の引き上げ／工夫した点＝曲とジャンプのタイミング合わせ・タイトル画面の背景／落下ゲームオーバー時の見た目が少しシュールなのは味としてそのまま残している／ゲームファイル・スクリーンショットは後日追加予定。画像・DL・リポジトリは未設定
...
|----|---------|-----|------|----------|------|----|----|
| jump-beat | あがれ！ジャンプビート | 2026 | Unreal Engine, C++, 課題制作：個人 | Unreal Engine / C++ | - | - | - |
