...
- Works に「あがれ！ジャンプビート」（2026）を追加し、**Works全体の一番上（最新）**に配置（`jump-beat`、2026グループ先頭＝ぺったんメイカーの前。カルーセル `order` も先頭）。タグ＝Unreal Engine / C++ / 課題制作：個人、env＝`Unreal Engine 5 / C++ / Python（BGM合成）`、制作期間＝1週間弱。desc は他作品と同じ「概要→ゲーム性→技術→工夫」構成の長文（6段落）: ①ぷちコン応募用に1週間弱で個人制作したリズムアクション／お題「だん」から「段」を軸に、曲の拍に合わせSPACEで跳び崩れる階段を登る ②押した瞬間の精度で進める段数が変わり拍を外すと足場にヒビ／銃を撃てる要素で裏拍を絡め操作難易度を引き上げ ③同期ズレをBGM側から解決＝Pythonで波形から自作合成し拍位置を厳密に0に、判定はフレーム経過でなく音の実再生位置基準 ④跳躍は段数によらず滞空時間が一定になるよう毎回重力を逆算／階段は毎回自動生成、詰み防止で穴の連結数を制約 ⑤工夫した点＝曲とジャンプのタイミング合わせ・タイトル画面の背景／落下ゲームオーバーのシュールな見た目は味として意図的に残置 ⑥ゲームファイル・スクリーンショットは後日追加予定。画像・DL・リポジトリは未設定
...
|----|---------|-----|------|----------|------|----|----|
| jump-beat | あがれ！ジャンプビート | 2026 | Unreal Engine, C++, 課題制作：個人 | Unreal Engine 5 / C++ / Python(BGM合成) | - | - | - |
