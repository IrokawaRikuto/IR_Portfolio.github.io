...
- モーダルに「制作期間」「チーム規模」欄を追加（`workData[id].period = {ja,en}` / `team = {ja,en}` がある作品のみ表示。`.work-detail-period` に「制作期間：」、`.work-detail-team` に「チーム規模：」形式。表示順は 制作年 → 制作期間 → チーム規模 → 担当 → 開発環境 → 受賞）。現在 period 設定済みは ConsoleSTG（1日）のみ、他は本人からの情報待ち
