...
- Works カルーセルを任天堂「ピックアップ」風に強化＋年を親コントロール化。①見切れピーク＝中央カードを大きく（`--wc-w` 62%/tablet74%/mobile86%）、左右の隣が見切れて並ぶ。3連トラック（list×3）＋中央コピー基準の `pos` で無限ループ、`transitionend` で座標を[n,2n)へ戻す。非中央は `opacity/scale` で減光。②カード＝スクショ画像(object-fit:cover)＋タイトルのみ（タグ削除）。③5秒ごとに自動で右へ（`AUTO_MS`、hover/タブ非表示で停止、操作で再スタート）。④年切替（`setYear`）＝カルーセルはフェード＋横スライド、下グリッドは選択年だけ表示にフェード。年号セレクタは一周ループ（2026で前→2024、2024で次→2026）（`showGrid`：`.work-card` を年で display 切替＋`.visible` 付与）。⑤年見出し(`.year-head`)は廃止（年はセレクタが示す）。n===1(2025)は単体・自動/矢印無効。Playwright 確認済み（tracks24/dots8/タグ0、年切替でラベル＆グリッド連動、カード→モーダル）
